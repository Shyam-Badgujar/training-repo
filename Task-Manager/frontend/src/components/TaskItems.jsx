...
import { getPriorityBadgeColor, getPriorityColor, MENU_OPTIONS, TI_CLASSES } from '../assets/dummy.jsx'
...
function TaskItems({ task, onRefresh, showCompleteCheckbox = true, onLogout }) {
      ...
      <div className={`${TI_CLASSES.wrapper} ${borderColor}`}>
        <div className={TI_CLASSES.leftContainer}>
          ...
          <div className='flex-1 min-w-0'>
            ...
            {/* ✅ TIMER ADDED HERE — only show if task is not completed */}
            {!isCompleted && <TaskTimer taskId={task._id} />}
          </div>
        </div>
        ...
        <div className={TI_CLASSES.rightContainer}>
              ...
              <div className={TI_CLASSES.menuDropdown}>
                {MENU_OPTIONS.map(opt => (
                  <button key={opt.action} onClick={() => handleAction(opt.action)}
                    className='w-full px-3 sm:p-4 py-2 text-left text-xs sm:text-sm hover:bg-blue-50 flex items-center gap-2'>
                    {opt.icon}{opt.label}
                  </button>
                ))}
              </div>
          ...
          <div>
            <div className={`${TI_CLASSES.dateRow} ${task.dueDate && isToday(new Date(task.dueDate)) ? 'text-blue-600' : 'text-gray-500'}`}>
              ...
              {task.dueDate
                ? (isToday(new Date(task.dueDate)) ? 'Today' : format(new Date(task.dueDate), 'MM/dd'))
                : '-'}
            </div>
            <div className={TI_CLASSES.createdRow}>
              <Clock className='w-3 h-3 sm:w-3.5 sm:h-3.5' />
              {task.createdAt ? `Created ${format(new Date(task.createdAt), 'MM/dd')}` : 'No date'}
            </div>
          </div>
        </div>
      </div>
